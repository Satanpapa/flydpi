...
func Open(coreDLL, observerDLL string) (*Runtime, error) {
	...
	if err != nil { return nil, fmt.Errorf("load core dll: %w", err) }
	start, err := dll.FindProc("flydpi_runtime_start")
	if err != nil { _ = dll.Release(); return nil, fmt.Errorf("runtime start symbol: %w", err) }
	poll, err := dll.FindProc("flydpi_runtime_poll")
	if err != nil { _ = dll.Release(); return nil, fmt.Errorf("runtime poll symbol: %w", err) }
	stop, err := dll.FindProc("flydpi_runtime_stop")
	if err != nil { _ = dll.Release(); return nil, fmt.Errorf("runtime stop symbol: %w", err) }

	path, err := syscall.BytePtrFromString(observerDLL)
	if err != nil { _ = dll.Release(); return nil, fmt.Errorf("observer path: %w", err) }
	ret, _, callErr := start.Call(uintptr(unsafe.Pointer(path)))
	if ret == 0 {
		_ = dll.Release()
		if callErr != syscall.Errno(0) { return nil, fmt.Errorf("runtime start: %w", callErr) }
		return nil, fmt.Errorf("runtime start failed")
	}
	return &Runtime{dll: dll, start: start, poll: poll, stop: stop, rt: ret}, nil
}
...
func (r *Runtime) Close() {
	...
	if r.dll != nil { _ = r.dll.Release(); r.dll = nil }
}
